...
function Header(props) {
    ...
    useEffect(() => {
        setInputLocation(props.searchLocation);
        setInputDateStay(props.searchDateStay);
        setInputDateExperience(props.searchDateExperience);
        setInputGuest(props.searchGuest);
    }, [props.searchLocation, props.searchDateStay, props.searchDateExperience, props.searchGuest]);
    ...
    return (
        <div className={styles.container} style={{ color: 'var(--black)' }} version='other' mode={parseInt(router.query.menu) === 0 ? 'stay' : 'experience'}>
            ...
            {/* logo */}
            <img className={styles.logo} src={logo} alt='airbnb-logo' onClick={(e) => getSearchResult(e, '/')} />
